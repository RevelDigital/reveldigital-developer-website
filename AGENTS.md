...
- **Build output directory**: `docs/` (configured via `site_dir` in `mkdocs.yml`; **not committed** — gitignored and deployed via GitHub Actions)
- **Configuration**: `mkdocs.yml`
- **Deployment**: [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) builds with `mkdocs build --strict` on push to `master` and deploys to GitHub Pages via `actions/deploy-pages`. Pull requests run the build as a validation check (no deploy).
- **Custom domain**: `developer.reveldigital.com`, preserved via `mkdocs/CNAME` which MkDocs passes through to the build output.
...
# Build the static site (outputs to docs/, which is gitignored)
mkdocs build --strict
```
...
- The `docs/` directory is build output only — gitignored and regenerated by CI on every push to `master`. Do not commit it.
- Non-Markdown files dropped in `mkdocs/` (e.g. `CNAME`) are passed through verbatim to the built site.
